...
import { CheckCircle, PlayCircle, LogOut, User as UserIcon, History, Calendar, Key, X, GraduationCap, Eye, ChevronDown, Moon, Sun, Layers, MousePointer2 } from 'lucide-react';
...
const WAEC_SUBJECTS = {
    'General Subjects': ['English', 'Mathematics', 'Civic Education'] as Subject[],
    'Science': ['Physics', 'Chemistry', 'Biology', 'Agricultural Science', 'Geography', 'Further Mathematics'] as Subject[],
...
export const SubjectSelection: React.FC<Props> = ({ onStartExam, hasSavedSession, onResume, onLogout, onReview, user, examType, setExamType, theme, toggleTheme }) => {
  ...
  useEffect(() => {
    if (user.username) {
        setHistory(getStudentResults(user.username));
    }
  }, [user]);
  ...
  const handleChangePassword = (e: React.FormEvent) => {
    ...
    try {
        changePassword(user.username, pwdData.old, pwdData.new, 'student');
  ...
  const renderSubjectButton = (sub: Subject) => (
    <button
        ...
        className={`
            ...
            ? isJamb ? 'bg-green-50 dark:bg-green-900/30 border-green-500 text-green-900 dark:text-green-100 ring-1 ring-green-500' : 'bg-blue-50 dark:bg-blue-900/30 border-blue-500 text-blue-900 dark:text-blue-100 ring-1 ring-blue-500'
        ...
        `}
    >
        <span className="truncate mr-2 font-semibold">{sub}</span>
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 flex flex-col items-center justify-center p-2 md:p-4 transition-colors duration-300">
      <div className={`bg-white dark:bg-gray-800 max-w-5xl w-full rounded-xl shadow-2xl overflow-hidden mb-8 relative border-t-8 transition-colors duration-500 ${isJamb ? 'border-green-600' : 'border-blue-600'}`}>
        
        {/* HEADER */}
        <div className={`p-4 md:p-6 text-white flex flex-col md:flex-row justify-between items-start md:items-center gap-4 transition-colors duration-500 ${isJamb ? 'bg-green-900' : 'bg-blue-900'}`}>
            <div className="flex items-center gap-3 md:gap-4">
                ...
                <div>
                    <h1 className="text-xl md:text-2xl font-bold tracking-tight">EBUS EDU CONSULT (EEC)</h1>
                    <p className="text-yellow-400 text-[10px] md:text-xs font-semibold uppercase tracking-wider">{examType} CBT Practice Portal</p>
                </div>
            </div>
            ...
            <div className="flex items-center gap-4 w-full md:w-auto justify-between md:justify-end border-t md:border-0 border-white/20 pt-3 md:pt-0">
                ...
                <div className="flex gap-2">
                    ...
                    <button onClick={onLogout} className="text-xs bg-red-600 px-4 py-2 rounded hover:bg-red-700 transition flex items-center gap-1 font-bold shadow-md">
                        <LogOut size={14}/>
                    </button>
                </div>
            </div>
        ...
        <div className="flex border-b border-gray-200 dark:border-gray-700">
            <button 
                ...
                className={`flex-1 py-4 text-center font-bold text-sm transition-all ${isJamb ? 'text-green-700 dark:text-green-400 border-b-4 border-green-600 bg-green-50 dark:bg-green-900/20' : 'text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300'}`}
            >
                JAMB UTME
            </button>
            <button 
                onClick={() => setExamType('WAEC')}
                className={`flex-1 py-4 text-center font-bold text-sm transition-all ${!isJamb ? 'text-blue-700 dark:text-blue-400 border-b-4 border-blue-600 bg-blue-50 dark:bg-blue-900/20' : 'text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300'}`}
            >
                WAEC SSCE
            </button>
        </div>
          ...
          <div>
            <div className="flex justify-between items-end mb-4 border-b border-gray-200 dark:border-gray-700 pb-2">
                <h2 className="text-lg md:text-xl font-bold text-gray-800 dark:text-gray-100 flex items-center gap-2">
                    ...
                    {isJamb ? 'Subject Combination' : 'Choose Subject'}
                </h2>
                <span className={`text-[10px] uppercase font-bold px-2 py-1 rounded ${isJamb ? 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300' : 'bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-300'}`}>
                    {isJamb ? 'Multi-Subject Exam' : 'Single Subject Exam'}
                </span>
            </div>
            ...
            <div className="mb-6">
                ...
                <div className="max-h-[400px] overflow-y-auto pr-2 custom-scroll space-y-4">
                    {isJamb ? (
            ...
            <Button 
                ...
                disabled={!isValid}
                className={`w-full py-4 text-lg font-bold shadow-md transition-all transform hover:scale-[1.01] flex items-center justify-center gap-2 ${
                    isValid 
          ...
          <div className="bg-gray-50 dark:bg-gray-700/30 p-4 md:p-6 rounded-xl border border-gray-200 dark:border-gray-700 flex flex-col h-full min-h-[300px]">
             ...
             {history.length === 0 ? (
                 ...
                 <div className="overflow-y-auto max-h-[400px] space-y-3 pr-2 custom-scroll">
                     {history.map((res) => {
                         const isResJamb = res.session.examType === 'JAMB' || !res.session.examType;
                         return (
                         <div key={res.id} className={`bg-white dark:bg-gray-800 p-4 rounded-lg border-l-4 ${isResJamb ? 'border-l-green-500' : 'border-l-blue-500'} border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow group relative`}>
                             <div className="flex justify-between items-start mb-2 border-b border-gray-100 dark:border-gray-700 pb-2">
                                 <div className="flex flex-col">
                                     ...
                                     <span className={`text-[10px] font-bold px-1 rounded w-fit mt-1 ${isResJamb ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300' : 'bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300'}`}>{res.session.examType || 'JAMB'}</span>
                                 </div>
        ...
        {showPwdModal && (
            <div className="absolute inset-0 bg-green-900/80 dark:bg-black/80 z-50 flex items-center justify-center backdrop-blur-sm p-4">
