...
import { Timer, ChevronLeft, ChevronRight, CheckSquare, Flag, Calculator as CalcIcon, X, Delete, GraduationCap, Grid, Maximize, Minimize, AlertCircle, Keyboard, MousePointer2, Moon, Sun } from 'lucide-react';
...
export const ExamSimulator: React.FC<Props> = ({ session: initialSession, onSubmit, user, theme, toggleTheme }) => {
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-100 dark:bg-gray-900 select-none relative transition-colors duration-300">
      
      {/* INSTRUCTIONS MODAL */}
      {showInstructions && (
          <div className="fixed inset-0 bg-green-900 z-[60] flex items-center justify-center p-4">
              <div className="bg-white dark:bg-gray-800 max-w-2xl w-full rounded-xl shadow-2xl overflow-hidden">
                  <div className="bg-green-800 dark:bg-green-900 p-6 text-white text-center border-b-4 border-yellow-500">
                      <GraduationCap size={48} className="mx-auto mb-2 text-yellow-400"/>
                      <h2 className="text-2xl font-bold uppercase tracking-wide">{session.examType} Instructions</h2>
                      <p className="text-green-200 text-sm">Please read carefully before proceeding</p>
                  </div>
                  
                  <div className="p-6 md:p-8 space-y-6">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg border border-gray-200 dark:border-gray-600">
                              <h3 className="font-bold text-gray-800 dark:text-white mb-3 flex items-center gap-2"><AlertCircle size={18} className="text-green-600 dark:text-green-400"/> Exam Rules</h3>
                          ...
                          <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg border border-gray-200 dark:border-gray-600">
                              <h3 className="font-bold text-gray-800 dark:text-white mb-3 flex items-center gap-2"><Keyboard size={18} className="text-green-600 dark:text-green-400"/> Keyboard Shortcuts</h3>
                      ...
                      <div className="flex items-center justify-between bg-yellow-50 dark:bg-yellow-900/20 p-4 rounded border border-yellow-200 dark:border-yellow-800">
                          <div>
                              <p className="text-xs text-yellow-800 dark:text-yellow-400 font-bold uppercase">Duration</p>
                              <p className="text-lg font-bold text-gray-800 dark:text-white">{Math.floor(session.durationSeconds / 60)} Minutes</p>
                          </div>
                          <div>
                              <p className="text-xs text-yellow-800 dark:text-yellow-400 font-bold uppercase">Total Questions</p>
                              <p className="text-lg font-bold text-gray-800 dark:text-white">{Object.values(session.questions).flat().length} Questions</p>
                          </div>
                      </div>

                      <Button onClick={handleStartExam} className="w-full py-4 text-lg bg-green-700 hover:bg-green-800 text-white shadow-lg border-green-800">
      ...
      {/* HEADER */}
      <header className="bg-white dark:bg-gray-800 border-b dark:border-gray-700 px-3 py-2 md:px-4 flex justify-between items-center shadow-sm border-t-4 border-yellow-500 z-20 transition-colors">
        <div className="flex items-center gap-2 md:gap-4">
          <div className="flex items-center gap-2 md:border-r border-gray-200 dark:border-gray-700 md:pr-4 md:mr-2">
          ...
          <div className="hidden sm:block overflow-hidden max-w-[120px] md:max-w-none">
        ...
        <div className="flex items-center gap-2 md:gap-4">
          <Button
            ...
            variant="outline"
            size="sm"
            className="px-2 py-1.5 md:px-3 md:py-1.5 items-center justify-center border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
          ...
          <Button 
            ...
            variant="outline"
            size="sm"
            className={`px-2 py-1.5 md:px-3 md:py-1.5 border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white ${showCalculator ? 'bg-blue-100 border-blue-300 text-blue-700 dark:bg-blue-900 dark:text-blue-300' : ''}`}
            title="Calculator"
          >
            <CalcIcon size={18} />
          </Button>
          ...
          <Button
            ...
            variant="outline"
            size="sm"
            className="md:hidden px-2 py-1.5 border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
          >
            <Grid size={18} />
          </Button>

          <div className="flex flex-col items-center bg-gray-50 dark:bg-gray-700 px-2 py-1 md:px-3 rounded border border-gray-200 dark:border-gray-600 min-w-[70px] md:min-w-[100px]">
            ...
            <div className={`text-base md:text-xl font-mono font-bold leading-none ${timeLeft < 300 ? 'text-red-600 animate-pulse' : 'text-green-700 dark:text-green-400'}`}>
          ...
          <Button 
            ...
            variant="danger" 
            size="sm"
            className="rounded px-3 py-1.5 md:px-6 md:py-2 bg-red-600 hover:bg-red-700 text-white font-bold shadow-sm text-xs md:text-sm"
          >
            Submit
          </Button>
        </div>
      ...
      {showCalculator && (
        <div className="absolute top-16 right-2 md:right-24 z-50 w-64 bg-gray-800 rounded-xl shadow-2xl border border-gray-600 overflow-hidden">
      ...
      <div 
        ref={tabsRef}
        className="bg-green-800 dark:bg-green-900 px-2 md:px-4 pt-2 flex gap-1 overflow-x-auto whitespace-nowrap custom-scroll"
      >
      ...
      <main className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 p-4 md:p-6 overflow-y-auto bg-white dark:bg-gray-800 m-0 md:m-2 rounded-none md:rounded shadow-sm border-0 md:border border-gray-200 dark:border-gray-700 flex flex-col">
          {q ? (
            <>
                <div className="flex flex-col sm:flex-row justify-between mb-4 border-b dark:border-gray-700 pb-2 gap-2">
                ...
                {/* ACTION BAR */}
                <div className="mt-6 md:mt-8 flex flex-col md:flex-row justify-between items-center pt-4 border-t dark:border-gray-700 gap-4 md:gap-0">
                    ...
                    <div className="flex gap-2 w-full md:w-auto">
                        ...
                        >
                            <Flag className="inline mr-1" size={16} /> {session.markedForReview.includes(q.id) ? 'Unmark' : 'Mark for Review'}
                        </button>
                    </div>
                </div>
            </>
